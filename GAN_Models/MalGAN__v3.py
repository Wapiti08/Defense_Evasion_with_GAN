...
class MalGAN():
    def __init__(self, blackbox='RF', same_train_data=1, filename=Path(config.root/'data.npz').as_posix()):
    ...
    def train(self, epochs, batch_size=32, is_first=1):
        ...
        for epoch in range(epochs):
            ...
            # Save best model
            if TPR < best_TPR:
                self.combined.save_weights(Path(config.root/'saves'/'malgan.h5').as_posix())
                best_TPR = TPR
